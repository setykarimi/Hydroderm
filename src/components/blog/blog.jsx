        ...
        <div>
            ...
            <div className="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-4">
                {blogs.map(({ logo, bg, date, title }) => (
                    <div className="rounded-xl shadow-sm" style={{ background: `url(${bg}) no-repeat`, backgroundSize: 'cover' }}>
                        <div className="bg-white p-2 w-16 h-16 rounded-tr-xl rounded-bl-xl flex justify-center items-center">
                            <img src={logo} alt="logo" />
                        </div>
                        <div className="flex justify-between p-4 mt-12 backdrop-blur-md	items-center rounded-b-xl" style={{background: 'rgba(49, 86, 164, 0.40)'}}>
                            <div>
                                <span className="block text-white text-xs">{date}</span>
                                <span className="block text-white mt-2">{title}</span>
                            </div>
